...
{   
  ...
  switch (topicoVar) 
  {
         ...
         case 2: //el mensaje es de funcionamiento
        {
          ...
          if (msjRecibido == "PrenderSplit")
          {
            aire->prenderSplit();           
          }  
          if (msjRecibido == "ApagarSplit")
          {
            aire->apagarSplit();           
          }
          if (msjRecibido == "SubirT")
          {
            aire->subirTemp();           
          }  
          if (msjRecibido == "BajarT")
          {
            aire->bajarTemp();           
          }         
          break;
...
//Calcula el valor numerico, despues del caracter '-'. devuelve -1 si no esta presente dicho caracter
//Ejemplo  "Cambiar-42", devuelve el entero 42
int calculoNumero(String cadena) {
  int i = 0;
  int num = -1;
  while (cadena[i] != '\0')
  {
...
void setup() {
  ...
  aire = new ControladorAire(D6,D5,D3,D2,D1,D0); 
  digitalWrite(BUILTIN_LED, HIGH);  
